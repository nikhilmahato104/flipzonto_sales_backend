...
app.get('/init-session', (req, res) => {
  ...
  res.json({ sessionInitialized: true });
});

app.post('/login', async (req, res) => {
  const { email, password } = req.body;
  const admin = await Admin.findOne({ email });
  if (!admin || !(await bcrypt.compare(password, admin.password))) return res.send('Invalid email or password');
  ...
  try {
    await transporter.sendMail({ from: process.env.EMAIL_USER, to: ['26krakashyadav@gmail.com','nikhilmahato104@gmail.com','chandankumarsharma8124@gmail.com'], subject, html: htmlMessage });
    for (const mobile of ['+919304260733', '+919661461412']) {
      await twilioClient.messages.create({ body: `🔐 Flipzonto OTP: ${otp} (Sent on ${formattedDate})`, from: process.env.TWILIO_PHONE, to: mobile });
    }
    console.log('OTP sent via email and SMS');
    res.render('otp');
  } catch (err) {
...
app.post('/verify-otp', (req, res) => {
  if (req.session.tempAuth && req.session.otp == req.body.otp) {
    req.session.isAuth = true;
    delete req.session.tempAuth;
    delete req.session.otp;
    return res.redirect('/');
...
app.get('/', authMiddleware, (req, res) => res.render('adminDashboard'));
const salesmanRoutes = require('./routes/salesman');
app.use('/salesman', salesmanRoutes);
const salesmanAuthRoutes = require('./routes/salesmanAuth');
app.use('/salesman-auth', salesmanAuthRoutes);
...
app.get('/orderfetchfromDB', async (req, res) => {
  try {
    const orders = await Order.find();
    res.render('orderfetchfromDB', { orders });
  } catch (err) {
    res.status(500).send('Server Error');
  }
});
...
app.get("/api/orders", async (req, res) => {
  try {
    ...
    if (!salesmanName) return res.status(400).json({ success: false, message: "Salesman name is required" });
    const orders = await Order.find({ salesmanName });
    res.json(orders);
  } catch (error) {
    console.error("Error fetching orders:", error);
    res.status(500).json({ success: false, message: "Error retrieving orders. Please try again." });
